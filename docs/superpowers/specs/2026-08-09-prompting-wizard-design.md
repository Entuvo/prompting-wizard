...
`SKILL.md` logic, in full:

1. Read `PROGRESS.md`. If absent, run `assessment.md` and write it.
...
- **Markdown only.** No scripts, no manifest dependency, no plugin marketplace assumption.
- **Relative paths only.** Never reference an absolute path or a harness-specific directory.
- **No harness-specific tool names in the instructions.** `SKILL.md` states capabilities in terms of what must happen, not which tool does it.
...
- **Tier A — dispatch available.** The harness can spawn an isolated agent (Claude Code subagent; Codex `spawn_agent` with `[features] multi_agent = true` in `~/.codex/config.toml`). The skill dispatches the prompt, captures the output verbatim, and shows it unedited.
...
- **Concept** — one lever, explained in under 200 words
- **Before/after pair** — one weak prompt and its strong counterpart, with the domain slot filled from the learner's `## Tasks`
- **Exercise brief at three tiers** — novice gets a partial scaffold to complete, working gets a bare brief, advanced gets a brief plus an adversarial constraint. Three short paragraphs, not three lessons.
...
| 7 min | Critique against the lever's rubric; a stronger version is written, run in a separate clean context, and both outputs shown side by side |
| 3 min | Learner names the one change that moved the output; logged |

The run step is the point of the design. The learner sees their prompt fail rather than being told it would.

## Rubrics

`rubrics.md` holds one rubric per lever, each a short checklist scored 1–5. Reused verbatim by every lesson that touches that lever, so critique stays consistent across 30 days and the same weakness gets the same name every time.
