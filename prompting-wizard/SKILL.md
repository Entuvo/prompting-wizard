...
1. Look for `PROGRESS.md` in the learner's working directory.
   - **Absent** → the file is missing either because the learner is new or because it was lost mid-course, and an absent file cannot tell you which. Ask. If they are starting fresh, read `assessment.md` and run it; writing `PROGRESS.md` ends the session. If they were mid-course, accept a day number they state and rebuild the file from it, or re-run the assessment if they prefer. Never silently restart at day 1. Say plainly that a rebuild from a day number cannot reconstruct the Day 0 baseline, so day 30's baseline-versus-current comparison will be lost; re-running the assessment is the only path that keeps it. Let the learner choose knowing that.
   - **Present but a required field is missing or unparseable** → name the field that failed and stop. Never guess `current_day`, and never silently restart at day 1. Fit the offered repair to the field that failed: if `current_day` is the field that failed, offer to accept a day number the learner states; for any other missing or unparseable field (`## Levers`, `## Tasks`, `level`, or `## Log`), offer to re-run the assessment instead — a day number cannot repair those. `## Tasks` with fewer than three entries is a failed field — offer to re-run Part 3 of the assessment rather than the whole thing. A `current_day` that is not a whole number from 1 to 31 is a failed field — offer to accept a day number the learner states. The `## Log` section must be present and its lines parseable, but a file rebuilt from a stated day number legitimately carries no Day 0 baseline line and no per-day lines; an empty `## Log` is not a failed field, and day 30 says so when it reaches the baseline comparison.
...
**6. Completion.** If the day file has a `## Completion` section, carry it out after the session's state update — step 6 of `## Every session`, not of this loop — so any scores it reports are current — whatever the learner's tier.
